...
static void HandleEndTurn_FinishBattle(void)
{
   ...
   if(FLAG_ADVENTURE_STARTED==1){
        //custom code to save the current map section
        gSaveBlock1Ptr->EncounteredRoutes[gMapHeader.regionMapSectionId]=1;
    }
    u32 j;

    //custom code to delete pokemon from party at end of battles
    for(i = 0; i < gPlayerPartyCount; i++){
        u16 currentHP = GetMonData(&gPlayerParty[i], MON_DATA_HP);
        if(currentHP<=0){
            //now we remove the pokemon's data from the party

            //solution loops the following pokemon back one
            for(j = i; j < gPlayerPartyCount-1; j++){
                gPlayerParty[j]=gPlayerParty[j+1];
            }
            //we also remove the last pokemon in the party here
